...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
	  ...
	  if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        for (uint32_t i = 0; i < p_evt->params.rx_data.length; i++)
					...
					if(ble_string_get[0] == 'd')
						
					{
						
						ble_debug(ble_string_get);
						
						uint16_t length = strlen((char*)ble_string_put);
						err_code = ble_nus_string_send(&m_nus, ble_string_put, &length);
						//flush buffer
					...
					else if(ble_string_get[0] == 'n')
					{
						uint16_t length = strlen((char*)device_name);
						err_code = ble_nus_string_send(&m_nus, device_name, &length);
						APP_ERROR_CHECK(err_code);
					}
